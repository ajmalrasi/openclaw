#!/usr/bin/env bash
set -euo pipefail
task_dir=/home/ajmalrasi/json-schema-phase4e-20260923
...
exec > >(tee -a "$task_dir/engine-fit.log") 2>&1
date -Is
echo PHASE4E_SCOPE=isolated-production-engine-fit
...
for ((attempt = 0; attempt < 90; ++attempt)); do
  if curl --fail --silent --max-time 2 http://127.0.0.1:11435/health | jq -e '.status == "healthy"' > "$task_dir/health.json"; then
    healthy=true
    break
  fi
  if ! kill -0 "$(cat "$task_dir/candidate.pid")" 2>/dev/null; then
    echo PHASE4E_CANDIDATE_EXITED_BEFORE_HEALTH=1
    break
  fi
  sleep 1
done
if [[ "$healthy" != true ]]; then
  echo PHASE4E_PRODUCTION_ENGINE_FIT=failed
  exit 1
fi
echo PHASE4E_PRODUCTION_ENGINE_HEALTH=passed
timeout --signal=TERM --kill-after=5s 90s \
  "$reference_dir/.venv/bin/python" "$task_dir/endpoint_gate.py"
echo PHASE4E_PRODUCTION_ENGINE_FIT=passed
