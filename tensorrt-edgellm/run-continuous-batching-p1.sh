...
task_dir=/home/ajmalrasi/continuous-batching-p1-20260914
...
# The only header change is a friend declaration; the pinned reference archive is ABI-compatible.
# Reuse its CUDA device-link object to avoid recompiling or modifying the production runtime.
...
timeout --signal=TERM --kill-after=5s 250s \
    "$task_dir/continuous_batching_probe" "$engine_dir" "$checkpoint_dir"
