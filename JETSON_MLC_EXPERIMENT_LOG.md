...
- Current serving state (2026-09-22): the TensorRT Edge-LLM model unit is inactive and port 11434 is unbound; systemd records an orderly stop on 2026-09-20. The correct watchdog timer is active but does nothing while the model unit is inactive. Phase 0 of the JSON-Schema dependency work did not stop or restart serving. The vLLM rollback unit remains inactive. No runnable MLC image, service, or compiled Qwen3.5 library remains installed after the earlier experimentation cleanup; retained MLC source and weight files alone are not sufficient to benchmark.
...
- 2026-09-23 00:19 IST: final status for the separate TensorRT Edge-LLM JSON-Schema Phases 1–3 campaign. Isolated candidate passed 23 native tests, 112 Jetson Python tests (four async skipped for absent plugin; 116 host tests passed), opt-in XGrammar CMake target, and endpoint checks for object/SSE/nested Unicode, invalid-schema 400, incomplete-output 500, concurrent independent schemas and a stream disconnect that returned to zero active requests before slot reuse. The candidate server stopped; ports 11435 and 11434 are unbound; TensorRT model service remains inactive and watchdog timer active. The production-sized engine did not fit this candidate's memory headroom; functional gate used the smaller existing batch-two engine. Phase 4 performance/memory/full-build/deployment qualification is pending. All attempts and evidence are in the TensorRT journal and `TENSORRT_EDGE_LLM_JSON_SCHEMA_PHASE1_3_RESULTS.md`. No MLC state or unrelated service changed and no benchmark was run.

- 2026-09-22 20:27–2026-09-23 00:15 IST: continuing TensorRT Edge-LLM JSON-Schema Phases 1–3 campaign, not an MLC experiment. Isolated Jetson native/Python candidate gate passed 23 C++ tests and 108 Python tests (four Jetson async tests skipped; host runs them). An endpoint gate on the smaller existing batch-two engine passed single-object JSON, SSE, 400 rejections, concurrent independent object/array schemas and follow-up generation; the production-sized engine could not load under current RAM headroom. The initial cancellation label was too strong: an active request remained in `/health`, so a stricter disconnect/slot-release test is pending. The candidate endpoint was stopped and port 11435 unbound; production model service remained inactive, watchdog timer active, unrelated services untouched. Failed build/endpoint/CMake attempts and corrections are retained in `TENSORRT_EDGE_LLM_EXPERIMENT_LOG.md` and `/home/ajmalrasi/json-schema-p1-p3-20260922/`. No MLC model, artifact or service changed, and no benchmark was run. Current unresolved gates are the final CMake check, final rebuilt candidate endpoint check and strict cancellation release.

- 2026-09-22 20:09 IST: user authorized the combined TensorRT Edge-LLM JSON-Schema Phases 1–3 campaign, with long Jetson work detached and no production cutover. Work began in isolated local branch `feature/json-schema-guided-decoding` from qualified continuous-batching commit `1496d34`; the deployed Jetson checkout and its two pre-existing modifications were not edited. Read-only Jetson status found the model service still inactive, approximately 6.0 GiB available RAM, and—unlike the 18:14 preflight—the correct `openclaw-tensorrt-watchdog.timer` now inactive/not listed by `list-timers`; this observation was not caused or corrected by the campaign. Initial host implementation added bounded strict schema validation, native request propagation, XGrammar C++ integration behind an opt-in build flag, per-physical-slot matcher ownership/reset, request-local incomplete/error isolation, and streaming/non-stream terminal handling. Focused host Python gates passed 16/16 and the broader request/runtime regressions passed 96/96. Native Jetson build and inference gates remain pending; no benchmark, model load, package install, service restart, or unrelated-service change has occurred in this campaign yet.

- 2026-09-22 18:14–18:27 IST: user authorized Phase 0 of TensorRT Edge-LLM JSON-Schema guided decoding and required long jobs to stay detached. Read the required MLC/TensorRT continuity records and shared API contract before work. Preflight found the TensorRT model unit already inactive since 2026-09-20, port 11434 unbound, the correct watchdog timer active, approximately 5.9 GiB available RAM, and the two pre-existing modified files in the Jetson Edge-LLM checkout unchanged. Evaluated XGrammar 0.2.7 (`82505d0d987c36a4209fb3d8571cf6b0f28b5acd`, Apache-2.0) in isolated `/home/ajmalrasi/xgrammar-phase0-20260922/`; PyPI supplied CPython 3.12 aarch64 wheels, while the native C++17 core built without Python/PyTorch or CUDA code. Attempt 1 retained: the repository default re-enabled Python bindings and configuration stopped on absent `tvm_ffi`; corrected with a build-directory config. Attempt 2 built the 24 MiB static library and all 73 upstream C++ tests passed, but the wrapper stopped on absent GNU `/usr/bin/time`; no package was installed. Attempt 3 retained: the first harness passed detector-only metadata to a deserializer requiring full metadata and aborted; corrected to the public direct tokenizer constructor. Final detached gate used the exact Qwen tokenizer (248,320 model-logit vocabulary, EOS 248046), accepted/completed valid Unicode JSON, rejected invalid JSON at token index 3, produced a 31,040-byte token mask, compiled representative schemas in 0–2 ms, used a 982,000-byte five-schema cache, and reported 106,624 KiB process high-water RSS. `PHASE0_GATE passed=1`. External `$ref` was only warned and relaxed by XGrammar, establishing the requirement for strict pre-admission API validation and a conservative supported subset. Evidence is retained remotely and under `artifacts/xgrammar-phase0-20260922/`; full result is `TENSORRT_EDGE_LLM_JSON_SCHEMA_PHASE0_RESULTS.md`. No model inference, benchmark, engine build, package install, Edge-LLM source change, service restart, or unrelated-service change occurred. Phase 1 single-request native integration is the next unresolved gate.

- 2026-09-16 IST: requested status check after the TensorRT Edge-LLM continuous-batching rollout. This is not an MLC benchmark or a new MLC conclusion. Read-only live verification on the Jetson returned TensorRT-owned model alias `openclaw`, healthy endpoint, maximum batch size two and `max_num_seqs=2`; graph counters were 3 captures, 707 replays and 74 eager executions. One request was active at the instant of the check, so no disruptive benchmark was started. The separate TensorRT journal/evidence contains the qualified legacy-versus-batched comparison: 33.97 versus 22.54 aggregate output tok/s (1.507x) on its matched two-request workload, while singleton latency remained within 4.3% of legacy. The current TensorRT service is active; the previously documented watchdog timer unit name was not present in the live user-systemd unit list, so its timer state must be reconciled separately before relying on the earlier “watchdog active” statement. No MLC source, container, model artifact or service changed.
- 2026-09-16 IST: requested capability inventory for the deployed TensorRT implementation. Read the authoritative TensorRT phase plan, experiment journal, validation scripts and retained evidence; no Jetson state changed. Implemented/deployed scope is native two-slot state ownership, qualified 128-token chunked prefill with a safe 64–128-token final-chunk policy, decode-first continuous scheduling, bounded FIFO admission, safe slot reuse, independent per-request generation controls, cancellation, bounded native output/stream buffers, HTTP/SSE ticket integration, tools/reasoning/logprobs/stops/EOS/overload handling, and three startup CUDA decode-graph captures. The endpoint explicitly reports `context_reuse: false`: there is no cross-request prompt-prefix/KV page retention or reuse. The existing two physical sequence slots retain their own KV/recurrent/convolution state only while their request is active; the state is released/reset before a slot is reused. No speculative decoding or MTP is enabled. See the TensorRT journal for numerical qualification and retained unsupported raw short-tail behavior.
- 2026-09-16 IST: correction to the preceding watchdog status entry. The live system uses `openclaw-tensorrt-watchdog.timer`, not the mistakenly queried `openclaw-tensorrt-edgellm-watchdog.timer`. Read-only verification finds the correct timer enabled and active (waiting), triggering `openclaw-tensorrt-watchdog.service` every five minutes. The earlier claimed deployment-observability discrepancy was caused by querying the wrong unit and is withdrawn. The same compiler/deployment inventory verified CUDA compiler `nvcc` 13.2.86 and CMake 3.28.3 on the Jetson; TensorRT Edge-LLM's native runtime/Python binding was built from the checked-out NVIDIA repository against the installed NVIDIA TensorRT/CUDA development libraries. No state changed.
- 2026-09-16 IST: requested upstream-PR provenance check for the TensorRT work. Read-only local Git inspection verified that user fork branch `codex/continuous-batching-p1` is pushed at `1496d3404f0cc18d26fb51d830991d28cc50cccb`; it is based directly on NVIDIA upstream `main`/release 0.10.1 commit `e8b29522938901f6df19ebeedd4b69bc8edbcd97`. The change stack has seven commits (persistent slots/steps, bounded chunked prefill, scheduler, per-sequence policies, Python/HTTP bridge and P7 graph/performance work) across 36 files with 5,166 additions and 26 deletions. No pull request, branch mutation, source change or Jetson change was made in this information check.
- 2026-09-16 IST: user explicitly requested removal of the `codex/` identity from the public feature-branch name. Renamed local branch `codex/continuous-batching-p1` to `feature/continuous-batching`, pushed it to the user fork, and deleted the old remote branch name. Verified the sole remaining remote feature reference is `feature/continuous-batching` at unchanged qualified commit `1496d3404f0cc18d26fb51d830991d28cc50cccb`; local branch tracks that remote. No pull request was opened; no source, engine or Jetson service changed.
- 2026-09-16 IST: user reported opening upstream NVIDIA TensorRT Edge-LLM pull request #215. Read-only GitHub API verification: PR title `feat: add native continuous batching and chunked prefill for LLM runtime`, open/non-draft against `main`, head `ajmalrasi:feature/continuous-batching` at qualified commit `1496d3404f0cc18d26fb51d830991d28cc50cccb`. GitHub currently reports it mergeable with state `clean`; no labels, requested reviewers, reviews or CI check runs are present yet. This records contribution state only; no Jetson source, engine or service changed.
- 2026-09-16 IST: user requested a short, current live benchmark for sharing. After a read-only idle-health gate, executed an API-only streaming benchmark on the deployed TensorRT service; no package (including iperf), compiler, engine, service configuration, restart or source change was used. Workload: deterministic 33-token prompt requesting numbered integers; one 8-token warmup; three sequential 32-token requests; simultaneous and staggered two-client 48-token requests. Elapsed benchmark time was approximately 11 seconds, within the five-minute cap. Sequential median: 23.63 decode tok/s, 1.479 s end-to-end latency, 124 ms TTFT. Simultaneous pair: 34.96 aggregate output tok/s over 2.746 s; individual decode rates 19.54/18.62 tok/s and TTFT 283/118 ms. Staggered pair: 35.07 aggregate output tok/s over 2.738 s; individual decode rates 18.70/19.56 tok/s and TTFT 124/157 ms. Post-run health was healthy/idle, with three graph captures and replay count increasing from 1,098 to 1,294. The current simultaneous 34.96 tok/s is 2.9% above P7 candidate 33.97 tok/s and 55.1% above the preserved legacy one-sequence P7 result 22.54 tok/s; use the latter as a same-workload historical comparison, while recognizing different-run variance. This is a short calibration, not a long-duration load/stability benchmark.
- 2026-09-16 IST: user challenged the prior statement that no four-concurrency engine existed. Read-only Jetson artifact inspection confirms that statement was incorrect. Complete engine bundles exist for batch one, batch two and batch four. In particular, `/home/ajmalrasi/Qwen3.5-4B/engines/llm-b4-input4096-kv4608-vanilla/` contains `llm.engine` and its config embeds `max_batch_size=4`, `max_input_len=4096`, `max_kv_cache_capacity=4608` and 144 KV pool pages. Historical retained evidence records successful synthetic native batch-four execution: at past KV 512 / 256 generated tokens, batch 4 delivered 17.56 tok/s per stream and 70.24 aggregate tok/s, without OOM. The currently deployed continuous-batching server still deliberately uses the separate batch-two 6144/8192 vanilla engine and exposes only `max_num_seqs=2`; neither four-way HTTP serving nor the continuous scheduler’s correctness/memory behavior on the batch-four artifact has been validated. Batch-four MTP bundle initialization previously OOMed; the `b4` non-speculative directory is empty after a failed build. No artifact, service or source changed during this correction.
- 2026-09-16 IST: user authorized a short direct load/one-forward smoke test of the batch-four vanilla engine, not an extensive validation. Idle preflight passed. Paused only the model service and watchdog, then invoked the repository `llm_inference` executable with the batch-four vanilla engine, checkpoint and one existing one-request test fixture (16-token generation cap); automatic restoration trap restarted the production batch-two service/timer. Attempt 1 failed before deserialization because the standalone tool did not inherit `EDGELLM_PLUGIN_PATH`; no forward occurred. Corrected retry set the existing service’s plugin path explicitly. It loaded all 192 external tensors, embedding and tokenizer; TensorRT deserialized the 1.974 GB engine successfully, created its execution context (1,862 MiB TensorRT-managed GPU allocation), and recognized `maxBatch=4`. Runtime then allocated fixed batch-four tensors and failed before first forward with `cudaMalloc ... out of memory` / `NvMapMemAlloc ... error 12`. This proves the artifact remains a valid/deserializable batch-four engine but cannot initialize its full four-slot runtime in the current Jetson memory state even for a single request. It does not contradict the historical synthetic benchmark, but supersedes any assumption that this present deployment can load it reliably. Evidence: `/home/ajmalrasi/b4-single-smoke-20260916/run.log`, `run-retry.log`, and `retry-result.txt`; no output JSON was produced. Production TensorRT batch-two service and watchdog were restored; after startup retries, `/health` was healthy/idle with batch capacity/two active sequences. No engine or source was modified.
- 2026-09-16 IST: user requested memory for a new shorter-input engine build and explicitly authorized stopping serving. Pre-stop health was healthy/idle; stopped only `openclaw-tensorrt-edgellm.service` and `openclaw-tensorrt-watchdog.timer`. Both report inactive afterward; no `experimental.server`, runtime binding, or `llm_inference` model process remained. Post-stop memory snapshot: 5,272,756 kB free / 5,750,788 kB available RAM and 16,178,172 kB swap free. Engine artifacts, source, API/database and other services were not changed. Serving remains intentionally stopped until the user starts it or asks for restoration.
- 2026-09-16 IST: user supplied newly built short-context batch-four engine prefix `llm-b4-input2048-kv4096-`; direct inspection found complete bundle `/home/ajmalrasi/Qwen3.5-4B/engines/llm-b4-input2048-kv4096-vanilla`. Embedded config verifies max batch 4, input 2048, KV capacity 4096 and 128 pool pages. With serving intentionally stopped, ran one bounded native four-request forward smoke test using existing ragged prompts and a six-token cap, with the deployed TensorRT Edge-LLM plugin path explicitly set. Engine deserialized, loaded weights/tokenizer, initialized fixed batch-four tensors and 714,213,376-byte shared execution memory, captured four decode graphs, and completed all four responses successfully. Summary: 138 prefill tokens in 312.34 ms (441.8 tok/s), 24 generated tokens at 81.7 aggregate GPU decode tok/s, 623.62 ms end-to-end (38.5 output tok/s). Peak reported `cuda_free_memory_delta` was 6,124.61 MB GPU / 6,604.42 MB CPU; these are monitor metrics, not additive residency. Output is coherent but truncated by the six-token cap. Evidence: `/home/ajmalrasi/b4-short-input4096-smoke-20260916/run.log` and `output.json`. This is a successful load/forward smoke test only, not HTTP, continuous-scheduler, quality, long-context or stability qualification. After exit, no inference/model process remained; serving and watchdog remain inactive as requested, with 5,959,200 kB available RAM.
- 2026-09-16 IST: interpretation requested for two-versus-four concurrency. The new 2048-input/4096-KV batch-four smoke result has no matched batch-two artifact/run, so it does not establish a 2→4 improvement by itself. Historical same-engine native results at 4096-input/4608-KV show batch two 43.96 aggregate decode tok/s versus batch four 70.24 (+59.8%); individual-stream decode fell 21.98→17.56 tok/s. Therefore four-way batching improved aggregate capacity materially but reduced per-request cadence; a controlled new short-context batch-two comparison is required before making a claim for the newly built engine. No state changed.
- 2026-09-16 IST: clarified that the new batch-four smoke’s end-to-end output throughput is 38.5 tok/s (24 generated tokens / 623.62 ms). It covers direct native prefill plus generation, but excludes one-time engine/weight/tokenizer load and HTTP/server overhead; the 81.7 tok/s figure is decode GPU time only. No state changed.
- 2026-09-16 IST: user requested longer outputs, interpreted as 512 generated tokens per each of four concurrent native requests. With serving still stopped, ran one bounded direct batch-four test using new `llm-b4-input2048-kv4096-vanilla`, four identical deterministic number-list prompts, and a 512-token cap. All four requests reached `finish=max-length`, exactly 512 outputs each (2,048 generated tokens total), with no OOM. Result: prefill 152 tokens in 266.26 ms (570.9 tok/s); GPU decode 2,048 tokens in 30,364.24 ms (67.4 aggregate tok/s); end-to-end direct-native prefill+generation 30,665.13 ms = **66.8 aggregate output tok/s**, or approximately 16.7 tok/s per concurrently active request. Cold engine/weight/tokenizer load and HTTP/server overhead remain excluded. Peak monitor values: 5,368.02 MB GPU / 6,576.03 MB CPU (`cuda_free_memory_delta`, not additive residency). Evidence: `/home/ajmalrasi/b4-input2048-kv4096-batch4-512-20260916/run.log`, `output.json`; input fixture is retained locally under `tensorrt-edgellm/evidence/b4-input2048-kv4096-20260916/batch4-512-input.json`. Serving/watchdog remain inactive as requested.
- 2026-09-16 IST: user requested testing newly built `/home/ajmalrasi/Qwen3.5-4B/engines/llm-b4-input4096-kv6144-vanilla`. Inspection verifies batch 4, input 4096, KV 6144 and 192 pages. With serving still stopped, attempted the same direct four-request/512-token test. It loaded 192 weights, embedding/tokenizer and successfully deserialized the engine (1,862 MiB TensorRT execution context) but failed before graph capture or forward during fixed batch-four runtime allocation: `NvMapMemAlloc ... error 12` / `cudaMalloc ... out of memory`. No output JSON. Evidence: `/home/ajmalrasi/b4-input4096-kv6144-batch4-512-20260916/run.log`. Thus the new 2048-input/4096-KV batch-four engine remains the validated fitting configuration; this larger 4096/6144 profile does not fit current 8 GB unified-memory conditions. Serving/watchdog remain intentionally inactive.
- 2026-09-16 IST: user requested retesting the existing batch-two production engine. With serving still stopped, ran one direct native batch of two identical deterministic prompts, each capped at 512 outputs, against `/home/ajmalrasi/Qwen3.5-4B/engines/llm-b2-input6144-kv8192-vanilla`. Engine/runtime initialized and both requests reached 512 tokens successfully (1,024 total). Results: prefill 76 tokens in 168.53 ms (450.9 tok/s); GPU decode 1,024 tokens in 24,505.74 ms (41.8 aggregate tok/s); end-to-end direct-native prefill+generation 24,737.90 ms = **41.4 aggregate output tok/s**, approximately 20.7 tok/s per active request. Reported monitor peak 6,425.29 MB GPU / 6,754.24 MB CPU (not additive residency). The batch-four short-context engine’s 66.8 aggregate tok/s is 61.4% higher, but the comparison is not profile-identical: batch two supports 6144 input/8192 KV while batch four supports 2048 input/4096 KV. Evidence: `/home/ajmalrasi/b2-input6144-kv8192-batch2-512-20260916/run.log`, `output.json`; retained local input fixture `tensorrt-edgellm/evidence/b2-input6144-kv8192-20260916/batch2-512-input.json`. Serving/watchdog remain inactive.
- 2026-09-16 IST: user selected batch two as final production configuration and authorized finalization. Restarted `openclaw-tensorrt-edgellm.service` and `openclaw-tensorrt-watchdog.timer`. After normal model-load startup, `/health` returned healthy/idle with TensorRT owner, three decode graph captures, `max_batch_size=2` and `max_num_seqs=2`; both service and timer are active. Updated final benchmark and agent-contract documentation to state that batch two (6144-input/8192-KV) is production, while batch-four artifacts are experimental and not HTTP-scheduler-qualified. No engine, source or deployment configuration was changed.
- 2026-09-16 IST: final live production smoke check after restart: OpenAI-compatible `POST /v1/chat/completions` with deterministic prompt `Reply exactly: READY` returned `READY` with `finish_reason=stop` (16 prompt / 2 completion tokens). `/health` immediately afterward was healthy/idle, with three graph captures, one replay, `max_batch_size=2` and `max_num_seqs=2`. This confirms the finalized batch-two TensorRT service, not merely systemd process health. Documentation finalization commit follows; no model configuration changed.
...
- 2026-09-11 after restoration commit: staged only the five declared campaign files. Whitespace validation passed for the journal, Dockerfiles and bug report; the patch artifact intentionally contains unified-diff blank context lines represented by a single leading space, which `git diff --check` reports as trailing whitespace even though those markers are required by that validated patch form. Created commit `a49d4aa` (`Document Jetson Qwen3.5 runtime experiments`) with 184 inserted lines across the five files and pushed it successfully to `origin/main`. A post-push fetch reported local and upstream `main` exactly synchronized at that commit, zero ahead and zero behind. The unrelated modified `.gitignore` and untracked `examples/` directory remained outside the commit. This journal entry will be committed and pushed separately so the repository record includes the commit/push action itself.
- 2026-09-11 after vLLM restoration: user requested exact current concurrency and context details. Live read-only verification found `openclaw-vllm-jetson.service` active since 19:05:13 IST with zero systemd restarts; container `openclaw-vllm-jetson` was up from the pinned NVIDIA image. The actual running command confirms `--max-num-seqs 1`, `--max-model-len 4096`, `--max-num-batched-tokens 1024`, and manual `--kv-cache-memory-bytes 192M`. `/v1/models` independently returned `openclaw` with `max_model_len=4096`. Startup logs quantify the hybrid cache as 1,056 GPU-KV tokens and explicitly report `Maximum concurrency for 4,096 tokens per request: 1.00x`; because Qwen3.5 uses recurrent GDN layers plus periodic full-attention layers, the 1,056 physical attention-cache-token figure must not be misread as the API context limit. Operational meaning: one active generation sequence at a time; simultaneous HTTP requests may be accepted/queued, but the engine will not execute two model sequences concurrently. The 4,096 limit is the total token budget for one request/sequence, including system prompt, conversation history, current input and generated output. The 1,024 value is a scheduler/prefill chunk ceiling, not the context window or request-concurrency setting. Current snapshot: container 6.158 GiB, system available RAM about 361 MiB, swap 3.0 GiB. No configuration or service state was changed during this status check.
- 2026-09-12: user requested the post-export deployment path for the new TensorRT Edge-LLM Qwen3.5-4B INT4 AWQ artifact. This is a new TensorRT Edge-LLM track, not a continuation or repair of the stopped MLC-mainline experiment. Evidence inspected: current Jetson AI Lab TensorRT Edge-LLM tutorial and the actual AWS export layout. The AWS container export uses TensorRT Edge-LLM `0.10.1` and contains `onnx/llm`, `onnx/visual`, and `onnx/mtp_draft`; the base config identifies `qwen3_5_text`, 32 layers, 24 Mamba/recurrent layers, 8 attention layers, FP16 KV cache, FP32 recurrent state, and MTP metadata. The ONNX-only Mac transfer was active at the status check with 2.3 GiB of 7.2 GiB present and 15 files copied. Proposed deployment sequence: finish and verify the ONNX transfer; copy the ONNX tree to the Orin; build the TensorRT Edge-LLM C++ runtime on JetPack 7.2/CUDA 13.2 for `jetson-orin`; stop vLLM and free model memory before the engine build; first build a conservative batch-one language engine from `onnx/llm` with 512 max input and 1024 KV capacity; optionally build the visual engine and MTP draft only after baseline text inference succeeds; run `llm_inference`, then a benchmark capped at five minutes. TensorRT engines must be built on the target Orin (SM87) and cannot be reused from the x86 AWS exporter. This is planning only: no Jetson files, services, engines, or runtime state were changed in this status turn.
- 2026-09-12 correction: the immediately preceding TensorRT Edge-LLM planning entry was placed in this MLC journal before the user clarified the record boundary. It is retained only because this journal is append-only; it must not be treated as part of the MLC experiment. All TensorRT Edge-LLM work, evidence, decisions, failures, builds, service changes, and benchmarks now belong in `TENSORRT_EDGE_LLM_EXPERIMENT_LOG.md`. Do not append further TensorRT Edge-LLM events here.
- 2026-09-12 15:02 IST: user requested shutdown of the already-running model. A remote `systemctl --user stop openclaw-vllm-jetson.service` confirmed the preserved vLLM unit inactive/dead with exit status 0; no vLLM container or vLLM engine process remained. This was a stop/idempotence check only: the active model endpoint was subsequently identified as the separately recorded TensorRT Edge-LLM service, so its shutdown evidence belongs in the TensorRT journal. No MLC artifact, container, or rollback configuration was changed.
